...
int getRuleLength(int ruleId) {
    ...
    case 14: return 5; // ASSIGN_STMT (Set...)
    case 15: return 5; // Increase...
    case 16: return 5; // Decrease...
    case 17: return 5; // Multiply...
    case 18: return 5; // Divide...
    ...
    case 25: return 7; // IF_STMT -> If COND then : BLOCK ELIF_LIST
    ...
    case 30: return 6; // LOOP_STMT -> Repeat E iterations : BLOCK
    ...
    case 64: case 65: case 66: case 67: case 68: case 69: return 1; // REL_OPs
    default: return 0;
    }
}
...
int getLHS(int ruleId) {
    ...
    if (ruleId >= 2 && ruleId <= 3) return NON_TERMINAL_STMT_LIST;
    if (ruleId >= 4 && ruleId <= 10) return NON_TERMINAL_STMT;
    if (ruleId == 11) return NON_TERMINAL_VAR_DECL;
    if (ruleId >= 12 && ruleId <= 13) return NON_TERMINAL_TYPE;
    if (ruleId >= 14 && ruleId <= 18) return NON_TERMINAL_ASSIGN_STMT;
    if (ruleId >= 19 && ruleId <= 20) return NON_TERMINAL_LVAL;
    if (ruleId >= 21 && ruleId <= 22) return NON_TERMINAL_IO_STMT;
    if (ruleId >= 23 && ruleId <= 24) return NON_TERMINAL_CTRL_FLOW;
    if (ruleId == 25) return NON_TERMINAL_IF_STMT;
    if (ruleId >= 26 && ruleId <= 27) return NON_TERMINAL_ELIF_LIST;
    if (ruleId >= 28 && ruleId <= 29) return NON_TERMINAL_ELSE_PART;
    if (ruleId >= 30 && ruleId <= 32) return NON_TERMINAL_LOOP_STMT;
    if (ruleId >= 33 && ruleId <= 34) return NON_TERMINAL_FUNC_DECL;
    if (ruleId >= 35 && ruleId <= 36) return NON_TERMINAL_PARAM_LIST;
    if (ruleId == 37) return NON_TERMINAL_FUNC_RET;
    if (ruleId == 38) return NON_TERMINAL_FUNC_CALL_STMT;
    if (ruleId >= 39 && ruleId <= 40) return NON_TERMINAL_FUNC_CALL;
    if (ruleId >= 41 && ruleId <= 42) return NON_TERMINAL_ARG_LIST;
    if (ruleId >= 43 && ruleId <= 44) return NON_TERMINAL_E;
    if (ruleId >= 45 && ruleId <= 47) return NON_TERMINAL_T;
    if (ruleId >= 48 && ruleId <= 52) return NON_TERMINAL_F;
    if (ruleId >= 53 && ruleId <= 54) return NON_TERMINAL_COND;
    if (ruleId >= 55 && ruleId <= 56) return NON_TERMINAL_BOOL_T;
    if (ruleId >= 57 && ruleId <= 59) return NON_TERMINAL_BOOL_F;
    if (ruleId >= 60 && ruleId <= 61) return NON_TERMINAL_E_OP;
    if (ruleId >= 62 && ruleId <= 63) return NON_TERMINAL_T_OP;
    if (ruleId >= 64 && ruleId <= 69) return NON_TERMINAL_REL_OP;
